...
final class DepartmentInputValidator
{
    ...
    /** @param array<string, mixed> $input @param array<string, mixed> $values @param array<string, string> $errors */
    private function optionalDescription(array $input, array &$values, array &$errors): ?string
    {
        $raw = $input['description'] ?? null;
        if ($raw === null || $raw === '') {
            $values['description'] = '';
            return null;
        }
        ...
        if (!is_string($raw)) {
            $values['description'] = is_scalar($raw) ? (string) $raw : '';
            $errors['description'] = 'Enter a text value.';
            return null;
        }
